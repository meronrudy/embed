...
use crate::embedded_network::EmbeddedNetwork;
...
pub struct RTICScheduler<T: FixedPoint + 'static> {
    /// Network instance
...
impl<T: FixedPoint + 'static> RTICScheduler<T> {
    ...
    pub fn execute_network_update(&mut self, inputs: &[T]) -> EmbeddedResult<Vec<FixedSpike<T>, 64>> {
